...
// Helpers
// ═══════════════════════════════════════════════════════════════════════════
async function fetchCompany(supabase, companyName) {
    const { data, error } = await supabase
        .from("companies")
        .select("id, name, tagline, mission, archetype, tone, avoid_phrases, voice_profile, editorial_guidelines, seo_content_guidelines, photography_style, color_primary, color_secondary, target_audiences, image_style_categories, reference_articles, include_toc")
...
function getAllBannedPhrases(company) {
...
function compileVoiceClause(company) {
    ...
    if (vp.pov_and_person)
        parts.push(`POV: ${vp.pov_and_person}`);
...
function buildPhotographyStyle(company) {
...
function buildColorPalette(company) {
...
function getStyleCategories(company) {
...
// GENERAL
// ─────────────────────────────────────────────────────────────────────────
server.tool("list_companies", "List all brands/companies configured in Organic.", {}, async () => {
...
// VOICE TOOLS
// ─────────────────────────────────────────────────────────────────────────
server.tool("get_voice_profile", "Get the full voice profile for a brand — tone, sentence rhythm, vocabulary, POV, rhetorical devices, sample phrases, and avoid patterns.", {
    ...
    if (!company) {
        return {
            content: [{ type: "text", text: `Company "${companyName}" not found.` }],
            isError: true,
        };
    }
    const profile = {
...
server.tool("get_banned_words", "Get the complete banned words/phrases list for a brand. Merges base Organic bans with company-specific avoid phrases and voice profile bans.", {
...
server.tool("get_style_rules", "Get the editorial and writing style rules for a brand — editorial guidelines, SEO content guidelines, structural dos/don'ts, specificity rules, and length rules.", {
    company: z.string().describe("The company/brand name"),
}, async ({ company: companyName }) => {
    const company = await fetchCompany(supabase, companyName);
    if (!company) {
...
server.tool("validate_tone", "Check a piece of text for banned phrases and voice profile violations. Returns any matches found.", {
    company: z.string().describe("The company/brand name"),
    ...
    const violations = allBanned.filter((phrase) => lower.includes(phrase.toLowerCase()));
    const result = {
        brand: company.name,
...
server.tool("compile_voice_prompt", "Compile a ready-to-use system prompt clause from a brand's voice profile. Inject this into any LLM prompt to write in the brand's voice.", {
    company: z.string().describe("The company/brand name"),
}, async ({ company: companyName }) => {
    const company = await fetchCompany(supabase, companyName);
    if (!company) {
...
// PHOTO STYLE TOOLS
// ─────────────────────────────────────────────────────────────────────────
server.tool("get_photography_style", "Get the brand's photography style — realism settings, lighting, mood, composition, subjects, and things to avoid.", {
    company: z.string().describe("The company/brand name (e.g. 'greencard-law')"),
}, async ({ company: companyName }) => {
    const company = await fetchCompany(supabase, companyName);
    if (!company) {
    ...
    return {
        content: [
            {
                type: "text",
                text: JSON.stringify({ brand: company.name, photography_style: photo }, null, 2),
            },
        ],
    };
});
server.tool("get_color_palette", "Get the brand's full color palette — primary, extended, and secondary colors with hex values.", {
    company: z.string().describe("The company/brand name"),
}, async ({ company: companyName }) => {
    const company = await fetchCompany(supabase, companyName);
    if (!company) {
...
server.tool("list_image_styles", "List all available image style categories for a brand — each with an id, label, narrative, and visual cues.", {
    company: z.string().describe("The company/brand name"),
}, async ({ company: companyName }) => {
    const company = await fetchCompany(supabase, companyName);
    if (!company) {
...
server.tool("get_image_style", "Get the full details of a specific image style category — including the detailed prompt style, storytelling cues, and composite settings.", {
    company: z.string().describe("The company/brand name"),
...
server.tool("generate_image_prompt", "Generate a complete image generation prompt for a blog post hero image, incorporating the brand's photography style, color palette, and selected image style category.", {
    company: z.string().describe("The company/brand name"),
    ...
    // Color palette
    parts.push(`- Brand colors: ${colors.primary.obsidian}, ${colors.primary.marigold}, ${colors.primary.sky}`);
    return {
        content: [
            {
                type: "text",
...
server.tool("get_composite_config", "Get the composite image configuration for a style category — background prompt, product query, and background image URL used by Organic's image compositing engine.", {
    company: z.string().describe("The company/brand name"),
    ...
    return {
        content: [
            { type: "text", text: JSON.stringify(composite, null, 2) },
        ],
    };
});
// ─────────────────────────────────────────────────────────────────────────
// BLOG TOOLS
// ─────────────────────────────────────────────────────────────────────────
server.tool("get_blog_system_prompt", "Compile the full blog system prompt for a brand — includes editorial credibility rules, anti-AI patterns, specificity rules, SEO targeting, FAQ/HowTo schema, sourcing rules, freshness signals, and the brand's voice profile. This is the exact prompt Organic uses internally.", {
    company: z.string().describe("The company/brand name"),
}, async ({ company: companyName }) => {
    const company = await fetchCompany(supabase, companyName);
    if (!company) {
    ...
    sections.push(`- Reinforce the primary keyword in the first 100 words, conclusion, and at least two H2s.`, 
    // Structure & Extraction
    `\n\n## Structure & Extraction`, `- Each H2/H3 must open with a 1-2 sentence factual answer quotable independently.`, `- Include "Key Takeaways" section (3-5 bullets) near the top.`, `- End with "Frequently Asked Questions" section (3-5 Q&A pairs as H3+p). MANDATORY.`, `- Em dash spacing: always "word — word", never "word—word".`, 
    // SEO
    `\n\n## SEO Targeting (MANDATORY)`, `- Identify one primary keyword and 5-15 secondary keywords.`, `- Match H2/H3 headings to real search queries.`, `- Win featured snippets: provide a direct 40-60 word answer for the primary keyword.`, `- meta_title: 50-60 chars, keyword + urgency/clarity.`, `- meta_description: 150-160 chars, keyword + value preview.`, `- slug: 3-6 words, lowercase, hyphenated.`, 
    // Sources
    `\n\n## Sources & Attribution (MANDATORY)`, `- Use "According to [linked source]" attributions. Minimum 4-6 hyperlinked citations per article.`, `- Link to the most specific page available, not homepages.`, 
    // Freshness
    `\n\n## Freshness Signals`, `- Reference ${currentYear} where relevant. Use "as of [date]" for time-sensitive facts.`, 
    // Structured data guidance
    `\n\n## Structured Data Guidance (MANDATORY)`, `- FAQ section: H3 > p pairing (for FAQPage schema).`, `- HowTo content: use ordered lists (for HowTo schema).`, 
    // Unique insight
    `\n\n## Unique Insight & Original Analysis (MANDATORY)`, `- Include decision frameworks, statistical breakdowns, edge cases, and practitioner-level guidance.`, `- At least one section must contain analysis not found on the primary government/reference source.`);
...
server.tool("get_editorial_guidelines", "Get the company-specific editorial guidelines — the custom writing framework that shapes voice, structure, depth, and content requirements.", {
    company: z.string().describe("The company/brand name"),
}, async ({ company: companyName }) => {
    const company = await fetchCompany(supabase, companyName);
    if (!company) {
        ...
        content: [
            {
                ...
                text: JSON.stringify({
                    brand: company.name,
                    editorial_guidelines: company.editorial_guidelines || "No custom editorial guidelines configured.",
...
server.tool("get_seo_guidelines", "Get the company-specific SEO content guidelines — keyword strategy, content depth rules, and search optimization framework.", {
    company: z.string().describe("The company/brand name"),
}, async ({ company: companyName }) => {
    const company = await fetchCompany(supabase, companyName);
    if (!company) {
        ...
        content: [
            {
                ...
                text: JSON.stringify({
                    brand: company.name,
                    seo_content_guidelines: company.seo_content_guidelines || "No custom SEO guidelines configured.",
                }, null, 2),
            },
        ],
    };
});
server.tool("get_blog_schema", "Get the JSON schema that Organic uses for structured blog generation output — title, excerpt, html, seo, faq, key_takeaways, how_to_steps, content_type. Use this to format your blog output correctly.", {}, async () => {
    const schema = {
        type: "object",
        required: [
            "title",
        ...
        properties: {
            ...
            html: {
                type: "string",
                description: "Full blog HTML content using h2/h3, lists, tables. Publication-ready.",
            },
            seo: {
                type: "object",
                properties: {
            ...
            how_to_steps: {
                type: "array",
                items: { type: "string" },
                description: "Ordered steps from procedural sections. Empty array if no steps.",
            },
            content_type: {
                type: "string",
...
server.tool("get_user_prompt_template", "Get the user prompt template that Organic uses for article generation — includes output requirements, JSON field mapping, and optional cluster context injection point.", {
    company: z.string().describe("The company/brand name"),
    ...
    if (!company) {
        return {
            content: [
                { type: "text", text: `Company "${companyName}" not found.` },
            ],
            isError: true,
        };
    }
...
server.tool("build_cluster_context", "Build cluster-aware context for article generation within a topical cluster. Handles keyword coordination, internal linking targets, and role-specific instructions (pillar vs supporting vs long-tail).", {
    ...
    const parts = [];
    const roleLabel = role === "pillar"
        ? "PILLAR PAGE (the hub)"
        : role === "supporting"
            ? "SUPPORTING PAGE (spoke)"
            : "LONG-TAIL PAGE (deep-dive spoke)";
    parts.push(`\n\n## Topical Cluster Context`);
    parts.push(`This article is part of the "${cluster_name}" content cluster.`);
    ...
    if (role === "pillar") {
        ...
        parts.push(`- Link OUT to every supporting and long-tail page`);
        parts.push(`- Cover each subtopic at a high level, deferring deep detail to spokes`);
        parts.push(`- Use anchor text matching each spoke's keyword target`);
    }
    else if (role === "supporting") {
        parts.push(`\nAs a supporting page:`);
        parts.push(`- Go deep on: "${page_keyword}"`);
        parts.push(`- Link BACK to the pillar: "${pillar_title}" (/${pillar_slug})`);
    ...
    parts.push(`\n### Keyword Coordination`);
    parts.push(`Other pages target these keywords — do NOT compete:`);
    for (const sib of sibling_pages) {
        parts.push(`- "${sib.keyword}" → "${sib.title}" (/${sib.slug})`);
    }
    // Internal linking
    parts.push(`\n### Internal Links (MANDATORY)`);
    parts.push(`Include natural links to these sibling pages using keyword-rich anchor text:`);
    for (const sib of sibling_pages) {
        parts.push(`- <a href="/${sib.slug}">${sib.keyword}</a> — "${sib.title}" (${sib.role})`);
    }
    parts.push(`Distribute links naturally — do not cluster in one section.`);
    return {
        content: [{ type: "text", text: parts.join("\n") }],
    };
});
server.tool("list_articles", "List existing articles for a brand — useful for finding internal linking targets, understanding content coverage, and avoiding topic duplication.", {
    company: z.string().describe("The company/brand name"),
        ...
        content: [
            {
                ...
                text: JSON.stringify({
                    ...
                    articles: articles.map((a) => ({
                        ...
                        content_type: a.seo?.content_type ?? null,
...
server.tool("get_article", "Get a single article by slug — returns full content, SEO data, FAQ, and outline. Useful for reviewing existing content or building internal link context.", {
    company: z.string().describe("The company/brand name"),
...
server.tool("create_article", "Save a new article to the Organic database. Accepts pre-generated article data (matching the blog schema from get_blog_schema) and inserts it into the articles table. Use get_blog_system_prompt + get_user_prompt_template to generate the content first, then use this tool to persist it.", {
    company: z.string().describe("The company/brand name"),
...
}, async ({ company: companyName, title, slug, excerpt, html, outline, seo, faq, key_takeaways, how_to_steps, content_type, model_used, image_style, image_prompt, image_base64, }) => {
    const company = await fetchCompany(supabase, companyName);
    if (!company) {
        return {
            content: [
...
// PROMPT TEMPLATES
// ═══════════════════════════════════════════════════════════════════════════
server.prompt("write_article", "Generate a brand-consistent blog article using the full Organic pipeline, with post-creation follow-up options.", {
...
server.prompt("brand_review", "Review and validate text against a brand's voice, tone, and editorial rules.", {
    company: z.string().describe("The company/brand name"),
...
server.prompt("onboard_brand", "Walk through setting up a new company's brand profile in Organic — voice, style, and editorial guidelines.", {
